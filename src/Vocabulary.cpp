...
#pragma omp parallel for num_threads(std::min(8, omp_get_max_threads()))
                ...
                std::cout << "[K-means] 开始计算 " << descriptors.size()
                          << " 个描述子到 " << clusters.size()
                          << " 个聚类中心的距离..." << std::endl;
...
#pragma omp single
                    {
                        ...
                        std::cout << "[K-means] 使用 " << num_threads
                                  << " 个线程并行计算距离" << std::endl;
                    }
...
#pragma omp for schedule(dynamic, 100)
                    ...
                    {
                        double best_dist =
                            DescManip::distance(descriptors[i], clusters[0]);
                        unsigned int icluster = 0;

                        for(unsigned int c = 1; c < clusters.size(); ++c)
                        {
                            double dist = DescManip::distance(descriptors[i],
                                                              clusters[c]);
                            if(dist < best_dist)
                            {
                                best_dist = dist;
...
#pragma omp for schedule(static)
                    for(unsigned int c = 0; c < clusters.size(); ++c)
                    {
                        ...
                        {
                            groups[c].insert(groups[c].end(),
                                             private_groups[t][c].begin(),
                                             private_groups[t][c].end());
                        }
                    }
                }

                std::cout << "[K-means] 距离计算完成，共有 "
                          << descriptors.size() << " 个描述子被分配到 "
                          << clusters.size() << " 个聚类中" << std::endl;
                ...
                if(!goon)
                {
                    std::cout << "[K-means] 聚类收敛，共迭代 " << iteration
                              << " 次" << std::endl;
                }
        ...
        if(current_level < m_L)
        {
            ...
            const std::vector<NodeId>& children_ids =
            ...
            for(unsigned int i = 0; i < clusters.size(); ++i)
            {
                ...
                {
                    std::cout << "[HKmeans] 开始处理第 " << current_level + 1
                              << " 层节点 " << (i + 1) << "/" << clusters.size()
                              << "，包含 " << child_features.size() << " 个特征"
                              << std::endl;
                                          ...
                                          std::vector<cv::Mat>& clusters) const
    {
        ...
        std::cout << "[K-means++] 开始选择 " << m_k << " 个聚类中心..."
                  << std::endl;
        ...
        int ifeature =
            rand() %
            pfeatures
                .size(); // DUtils::Random::RandomInt(0, pfeatures.size()-1);
        ...
        while((int)clusters.size() < m_k)
        {
            // 2.
            dit = min_dists.begin();
            for(auto fit = pfeatures.begin(); fit != pfeatures.end();
                ++fit, ++dit)
            {
                if(*dit > 0)
                {
            ...
            if(dist_sum > 0)
            {
                ...
                clusters.push_back(pfeatures[ifeature]);
                std::cout << "[K-means++] 已选择 " << clusters.size() << "/"
                          << m_k << " 个聚类中心 ("
                          << (clusters.size() * 100 / m_k) << "%)" << std::endl;
            } // if dist_sum > 0
            else
                break;
...
#pragma omp parallel for num_threads(num_threads)
            for(int i = 0; i < features.size(); i++)
            {
                ...
                if(w > 0) thread_vectors[thread_id].addWeight(id, w);
            }

            // 合并所有线程的结果
            for(auto& tv : thread_vectors)
            {
                for(auto& p : tv) { v.addWeight(p.first, p.second); }
            }
...
#pragma omp parallel for num_threads(num_threads)
            for(int i = 0; i < features.size(); i++)
            {
                ...
                WordValue w;
                transform(features[i], id, w);
                if(w > 0) thread_vectors[thread_id].addIfNotExist(id, w);
            }

            // 合并所有线程的结果 - 对于IDF和BINARY，使用addIfNotExist
            for(auto& tv : thread_vectors)
            {
                for(auto& p : tv) { v.addIfNotExist(p.first, p.second); }
            }
        }

        // 执行归一化(如需要)
        if(must_normalize) v.normalize(norm);
    }
        ...
        if(feature.type() == CV_8U)
        {
            do {
                ...
                for(const auto& id : nodes)
                {
                    ...
                    uint64_t dist = DescManip::distance_8uc1(
                        feature, m_nodes[id].descriptor);
                    if(dist < best_d)
                    {
                        best_d   = dist;
                        final_id = id;
        ...
        else
        {
            do {
                ...
                for(const auto& id : nodes)
                {
                    ...
                    //   m_nodes[id].descriptor<<std::endl;
                    uint64_t dist =
                        DescManip::distance(feature, m_nodes[id].descriptor);
                                      ...
                                      std::vector<WordId>& words) const
    {
        ...
        if(m_nodes[nid].isLeaf()) { words.push_back(m_nodes[nid].word_id); }
        else
        {
            ...
            while(!parents.empty())
            {
                ...
                const std::vector<NodeId>& child_ids =
                          ...
                          bool               binary_compressed) const
    {
        ...
        if(filename.find(".yml") == std::string::npos)
        {
            ...
            if(!file_out)
            {
                std::cout << "[Vocabulary] 错误: 无法打开文件进行写入: "
                          << filename << std::endl;
                throw std::runtime_error(
                    "Vocabulary::saveBinary Could not open file :" + filename +
                    " for writing");
            }
            std::cout << "[Vocabulary] 文件已打开，开始写入二进制数据..."
                      << std::endl;
        ...
        else
        {
            ...
            if(!fs.isOpened())
            {
                std::cout << "[Vocabulary] 错误: 无法打开文件进行YAML写入: "
                          << filename << std::endl;
        ...
        if(test_file.good())
        {
            ...
            test_file.close();
            std::cout << "[Vocabulary] 确认：文件已成功创建，大小为 "
                      << fileSize << " 字节" << std::endl;
        }
        else
        {
            std::cout << "[Vocabulary] 警告：无法打开已保存的文件进行验证"
                      << std::endl;
        }
    ...
    void Vocabulary::load(const std::string& filename)
    {
        ...
        if(!load(ifile))
        {
            ...
            {
                std::cout << "[Vocabulary] 检测到txt格式，使用文本加载器..."
                          << std::endl;
                load_fromtxt(filename);
            }
            else
            {
                std::cout << "[Vocabulary] 尝试使用OpenCV YAML格式加载..."
                          << std::endl;
    ...
    bool Vocabulary::load(std::istream& ifile)
    {
        ...
        {
            std::cout << "[Vocabulary] 非二进制格式 (magic number不匹配)"
                      << std::endl;
            return false;
        }
    ...
    {
        std::cout << "[Vocabulary] 尝试使用内存映射加载: " << filename
                  << std::endl;
        ...
        std::cout << "[Vocabulary] 文件大小: " << sb.st_size << " 字节"
                  << std::endl;
    ...
    {
        std::cout << "[Vocabulary] 开始从文本文件加载: " << filename
                  << std::endl;
        ...
        int expected_nodes =
            (int)((pow((double)m_k, (double)m_L + 1) - 1) / (m_k - 1));
        std::cout << "[Vocabulary] 预期节点数量: " << expected_nodes
                  << std::endl;
        ...
        while(!ifile.eof())
        {
            ...
            if(counter++ % 100 == 0)
            {
                std::cout << "[Vocabulary] 已处理 " << counter << " 行..."
                          << std::endl;
            }
    ...
    void Vocabulary::fromStream(std::istream& str)
    {
        ...
        if(compressed)
        {
            ...
            for(uint32_t i = 0; i < numChunks; i++)
            {
                ...
                // 解压数据
                size_t decompressed_size = qlz_decompress(
                    &compressed[0], &decompressed[0], &state_decompress);
                ...
                if(i % 10 == 0 || i == numChunks - 1)
                    std::cout << "[Vocabulary] 已解压 " << i + 1 << "/"
                              << numChunks << " 块, 总计 " << total_decompressed
                              << " 字节" << std::endl;
            }

            std::cout << "[Vocabulary] 解压完成，开始读取数据结构..."
                      << std::endl;
        ...
        for(size_t i = 1; i < m_nodes.size(); ++i)
        {
            ...
            node_count++;
            if(node_count % 100 == 0)
                std::cout << "[Vocabulary] 已读取 " << node_count
                          << " 个节点..." << std::endl;
        }

        std::cout << "[Vocabulary] 共读取 " << node_count << " 个节点"
                  << std::endl;
    ...
    {
        std::cout << "[Vocabulary] 从OpenCV FileStorage加载: " << name
                  << std::endl;
        m_words.clear();
        m_nodes.clear();
        ...
        for(unsigned int i = 0; i < fn.size(); ++i)
        {
            ...
            word_count++;
            if(word_count % 100 == 0)
                std::cout << "[Vocabulary] 已读取 " << word_count
                          << " 个词汇..." << std::endl;
        }

        std::cout << "[Vocabulary] OpenCV格式加载完成。词汇数量: "
                  << m_words.size() << ", 节点数量: " << m_nodes.size()
                  << std::endl;
    }

    void Vocabulary::save(cv::FileStorage& f, const std::string& name) const
    {
        std::cout << "[Vocabulary::save] 开始将词汇表写入OpenCV FileStorage: "
                  << name << std::endl;
        ...
        std::cout << "[Vocabulary::save] 已写入基本参数: k=" << m_k
        ...
        while(!parents.empty())
        {
            ...
            for(pit = children.begin(); pit != children.end(); pit++)
            {
                ...
                // 每处理100个节点打印一次进度
                if(node_count % 100 == 0)
                    std::cout << "[Vocabulary::save] 已处理 " << node_count
                              << " 个节点..." << std::endl;
            }
        }
    ...
    void Vocabulary::toStream(std::ostream& out_str, bool compressed) const
    {
        ...
        if(nnodes == 0)
        {
            std::cout << "[Vocabulary::toStream] 空词汇表，写入完成"
                      << std::endl;
            return;
        }
        ...
        std::cout << "[Vocabulary::toStream] 完成节点数据写入，共 "
                  << node_count << " 个节点" << std::endl;
        ...
        if(compressed)
        {
            ...
            std::cout << "[Vocabulary::toStream] 将分成 " << nChunks
                      << " 个块进行压缩" << std::endl;
            ...
            while(total_size != 0)
            {
                ...
                uint64_t compressed_size = qlz_compress(
                    &input[0], &compressed[0], readSize, &state_compress);
                total_size -= readSize;
                ...
                if(chunk_idx % 10 == 0 || total_size == 0)
                    std::cout << "[Vocabulary::toStream] 已压缩 " << chunk_idx
                              ...
                              << " 块，当前压缩率: " << std::fixed
                              << std::setprecision(2)
                              << (100.0 - (total_compress_size * 100.0 /
                                           (chunk_idx * chunkSize)))
                              << "%" << std::endl;
            }
        ...
        {
            std::cout << "[Vocabulary::toStream] 不压缩，直接写入数据"
                      << std::endl;
