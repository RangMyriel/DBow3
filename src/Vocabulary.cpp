...
namespace DBoW3
{
                               ...
                               int current_level)
  {
    ...
    else
    {
      ...
      constexpr int kKmeansMaxIterations = 256;
...
#pragma omp for schedule(static)
        ...
        std::cout << "[K-means] 非空聚类数: " << non_empty_clusters << "/"
        ...
        {
          goon = !std::equal(current_association.begin(),
                             current_association.end(),
                             last_association.begin(),
                             last_association.end());

          if (goon && !previous_association.empty() &&
              std::equal(current_association.begin(),
                         current_association.end(),
                         previous_association.begin(),
                         previous_association.end()))
          {
            std::cout
                << "[K-means] 检测到两步振荡，停止迭代并保留当前聚类结果"
                << std::endl;
            goon = false;
          }
        }

        if (goon && iteration >= kKmeansMaxIterations)
        {
          std::cout << "[K-means] 达到最大迭代次数 " << kKmeansMaxIterations
                    << "，停止迭代并保留当前聚类结果" << std::endl;
          goon = false;
        }
        ...
        if (goon)
        {
          ...
          previous_association = last_association;
