...
namespace DBoW3
{
    ...
    double DescManip::distance(const cv::Mat& a, const cv::Mat& b)
    {
        ...
        if(a.type() == CV_8U)
        {
            ...
            // This implementation assumes that a.cols (CV_8U) %
            // sizeof(uint64_t) == 0
    ...
    void DescManip::fromString(cv::Mat& a, const std::string& s)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for(int i = 0; i < a.cols; ++i, ++p)
                    if(!(ss >> *p))
                        cerr << "Error reading. Unexpected EOF. "
                                "DescManip::fromString"
                             << endl;
            }
        }
    }
                             ...
                             cv::Mat&                    mat)
    {
        ...
        else
        {
            ...
            for(int i = 0; i < N; ++i)
                memcpy(mat.ptr<float>(i),
                       descriptors[i].ptr<float>(0),
                       sizeof(float) * L);
        }
    }
    ...
    uint32_t DescManip::distance_8uc1(const cv::Mat& a, const cv::Mat& b)
    {
        ...
        // Bit count function got from:
        // http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetKernighan
        // This implementation assumes that a.cols (CV_8U) % sizeof(uint64_t) ==
        // 0
    ...
    double DescManip::distance_32fc1(const cv::Mat& a, const cv::Mat& b)
    {
        // 确保描述子的尺寸相同
        CV_Assert(a.cols == b.cols && a.rows == b.rows &&
                  a.type() == b.type() && a.type() == CV_32F);
    ...
    double DescManip::manhattan_distance_32fc1(const cv::Mat& a,
                                               const cv::Mat& b)
    {
        double       dist = 0.0;
        const float* pa   = a.ptr<float>(0);
        ...
        for(int i = 0; i < a.cols; ++i) { dist += std::abs(pa[i] - pb[i]); }

        return dist;
    }

    double DescManip::cosine_distance_32fc1(const cv::Mat& a, const cv::Mat& b)
    {
