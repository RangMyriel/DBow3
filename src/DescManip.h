...
namespace DBoW3
{
    ...
    class DBOW_API DescManip
    {
      public:
        ...
        // 欧氏距离(L2范数)
        static double distance_32fc1(const cv::Mat& a, const cv::Mat& b);
        // 曼哈顿距离(L1范数)
        static double manhattan_distance_32fc1(const cv::Mat& a,
                                               const cv::Mat& b);

        // 余弦距离 (适用于归一化描述子i)
        static double cosine_distance_32fc1(const cv::Mat& a, const cv::Mat& b);
