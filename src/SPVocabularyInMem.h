...
{
    SPDescribeMem()
        : cols(SP_DESC_COLS), rows(SP_DESC_ROWS), type(SP_DESC_TYPE)
    {
...
{
    SPNodeMem()
        : id(0), parent(0), word_id(0), weight(0)
    {
...
{
    SPVocMem()
        : magic_number(DBOW3_MAGIC_NUM), compressed(false), nnodes(0), k(0), L(0), scoring_type(0), weighting_type(0)
    {
        memset(payload, 0, sizeof(payload));
    }
    ~SPVocMem() {}
    ...
    static bool to_file(const DBoW3::Vocabulary &voc, const char *filename)
    {
        ...
        // Vocabulary 根节点本来就不携带有效描述子，这里保留默认零值即可。
