    ...
    class FastSearch
    {
      public:
        ...
        void                              create(Vocabulary& voc)
        {
            ...
            // consider possible aligment of each descriptor adding offsets at
            // the end
            ...
            _child_off_start =
                _feature_off_start +
                voc.m_k * _desc_size_bytes; // where do children information
                ...
                *reinterpret_cast<uint32_t*>(_data + block_offset_bytes) =
                ...
                for(const auto& c : Block.second)
                {
                    ...
                    memcpy(_data + block_offset_bytes + _feature_off_start +
                           ...
                           descsize);
                    assert(block_offset_bytes + idx * _desc_size_bytes +
                               descsize <
                           _total_size);
                ...
                for(const auto& c : Block.second)
                {
                    ...
                    if(!node.isLeaf())
                    {
                        ...
                        ptr_child->id_or_childblock =
                            block_offset[node.id]; // childblock
                    }
                    else
                    {
                        ...
                        ptr_child->id_or_childblock |=
                            0x80000000; // set the msb to one to distinguish
                                        // from offset
            ...
            cout << "nblocks used=" << currblock << " reserved=" << _nblocks
                 << endl;
        }
            ...
            for(int i = 0; i < desc.rows; i++)
            {
                ...
                while(!done)
                {
                    node_info* ni; //=getBestOfBlock(block,desc.row(i));
                    {
                        ...
                        uint32_t mind = std::numeric_limits<uint32_t>::max();
                        uint32_t n =
                            *reinterpret_cast<uint32_t*>(_data + block_start);
                        ...
                        for(i = 0; i < n4; i += 4)
                        {
                            uint64_t* ptr = (uint64_t*)(_data + _toff +
                                                        _desc_size_bytes * i);
                            ...
                            uint64_t* ptr_2 =
                                (uint64_t*)(_data + _toff +
                                            _desc_size_bytes * (i + 1));
                            uint32_t d2 =
                        ...
                        ni = (node_info*)(_data + block_start +
                                          _child_off_start +
                                          sizeof(node_info) * bestIdx);
                    }
                    if(ni->isleaf())
        ...
        uint64_t _child_off_start; // into the block,where the children offset
                                   // part starts
        ...
        void saveToFile(const std::string& filepath)
        {
            ...
            str.write((char*)&_block_size_bytes_al,
                      sizeof(_block_size_bytes_al));
